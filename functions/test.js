exports.processsEmailRequest = onRequest(async (req, res) => {
    ...
    const { mailbox_id, lead, batch_id, campaign_id, send_date } = req.body;
    ...
    try {
      ...
      try {
        ...
        return res.status(500).send("Email generation failed");
      }
      ...
      sendQueue[batch_id].leads.push({ mailbox_id, lead, subject, body: generatedEmail });
  
      await campaignRef.update({ sendQueue });
      console.log(`✅ Email stored in batch ${batch_id} for ${lead.email}`);
  
      console.log("Updated sendQueue:", JSON.stringify(sendQueue, null, 2));
  
    
      const updatedCampaignSnapshot = await campaignRef.get();
      console.log("Updated campaign document:", JSON.stringify(updatedCampaignSnapshot.data(), null, 2));
  ...
  exports.generateMail = functions.https.onRequest(async (req, res) => {
    try {
